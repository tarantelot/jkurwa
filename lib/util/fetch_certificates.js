...
function getUserInfo(signKeyFilePath, signKeyFile, password){
    return fetch(signKeyFilePath, signKeyFile, password).then(response => {
...
function fetch(signKeyFilePath, signKeyFile, password){
    // if (signKeyFile == null){
    //     signKeyFile = fs.readFileSync(signKeyFilePath)
    // }
...
function _preprocessCertificate(cert){
    ...
    const key_type          = subjectInfo.commonName.toLowerCase().includes('печатка') ? 'stamp': 'regular'
