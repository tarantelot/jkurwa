...
function getUserInfo(signKeyFile, password){
    return fetch(signKeyFile, password).then(response => {
        const preprocessedData = response
            .filter(item => item != null)
            .map(serverResponse => {
                return serverResponse.map(certificate => _preprocessCertificate(certificate))
        })
...
function fetch(signKeyFile, password){
    try{
        var box = new Box({
            keys: [{
                file: signKeyFile,
...
function _preprocessCertificate(certificate){
