...
function _getResponse(resp) {
    ...
    certificates.map(function (cert, index) {
        if (cert.extension.ipn != undefined){
            ...
            console.log(`CERTIFICATE: ${cert.as_dict()}`)
        }
    });
...
function post(queryUrL, data, response) {
    ...
    var req = http.request({
        ...
        res.on('end', function () {
            ...
            return response({body: complete_response_data, code: res.status});
        });
    });
...
function fetch(signKeyFile, password){
    ...
    for (idx=0; idx < servers.length; idx++) {
        return post(servers[idx] + '/services/cmp/', payload, _getResponse);
    }
